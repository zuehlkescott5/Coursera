...
The purpose of this report is to run simulations against the exponential distribution, therefore confirming the bootstrap method works, by checking sample distribution mean and spread, obtained by simulation, against the theoreteical mean and spread.
...
trueMean <- 1/lambda
pct_diff_mean <- (simMean - trueMean) / trueMean
simMean
trueMean
pct_diff_mean
```
...
pct_diff_sd <- (expSimsSD - trueSD) / trueSD
expSimsSD
trueSD
pct_diff_sd
```
...
trueVar <- trueSD^2
pct_diff_var <- (expSimsVar - trueVar) / trueVar
expSimsVar
trueVar
pct_diff_var
```
