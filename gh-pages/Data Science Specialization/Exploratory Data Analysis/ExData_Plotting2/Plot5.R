...
NEI.subset <- NEI[NEI$fips=="24510" & NEI$type=="ON-ROAD",  ]

totalByYear <- aggregate(Emissions ~ year, NEI.subset, sum)


png("ExData_Plotting2/plot5.png", width=640, height=640)
plt <- ggplot(totalByYear, aes(factor(year), Emissions))
plt <- plt + geom_bar(stat="identity", fill='red') +
  xlab("year") +
  ylab(expression('PM 2.5 emission')) +
  theme(legend.position='none') +
  ggtitle('Total Emissions from motor vehicles in Baltimore City, Maryland (1999 - 2008 by 3 year intervals)')
print(plt)
dev.off()